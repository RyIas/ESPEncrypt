...
#define ACK_TIMEOUT 500 // ms between resend attempts
...
void onDataRecv(const esp_now_recv_info_t *info, const uint8_t *data, int len) { 
  ...
  switch (incomingType) {
    ...
    case MSG_DISCOVERY: {
      ...
      if (isBlocked(info->src_addr, incoming.username, incoming.publicKey)) break; //if its a blocked device, dont read it.
      ...
      for(int i=0; i < discoveredCount; i++){ // goes from i=0 all the way up to however many discovered devices there are to check if this mac is any of the alr stored ones.
        if(memcmp(discoveredDevices[i].mac,info->src_addr,6)==0){ //compares the mac address against stored ones
      ...
      if(existingIndex != -1){ //if its not -1 update the new discoveredDevices[existingIndex] value. Useful as also updates already seen devices. If a device is factory reset it's MAC is not so it's good to update info.
        memcpy(discoveredDevices[existingIndex].mac,info->src_addr,6);
        ...
        discoveredDevices[existingIndex].rssi = info->rx_ctrl->rssi;
    ...
    case MSG_PAIR_REQUEST: {
      ...
      bool isThisAResponse = (deviceMode == MODE_PENDING && memcmp(pairingMac, info->src_addr, 6) == 0);
      ...
      if (isThisAResponse == false){ 
        ...
        memcpy(pairingMac, info->src_addr, 6);
        ...
        memcpy(theirRandNum, incoming.randNum, 64);

        for(int i = 0; i < 64; i++) myRandNum[i] = esp_random() & 0xFF;

        //makes peer struct
        esp_now_peer_info_t peer = {};
        memcpy(peer.peer_addr, pairingMac, 6);
        peer.channel = 0;
        peer.encrypt = false;

        //ensures the peer doesnt exist before adding as a peer to stop esp-now errors.
        if(!esp_now_is_peer_exist(pairingMac)) esp_now_add_peer(&peer);
        ...
        memcpy(outgoing.randNum, myRandNum, 64);

        //sends out response pair packet.
        esp_now_send(pairingMac, (uint8_t*)&outgoing, sizeof(outgoing));
    ...
    case MSG_CONFIRM: {
      ...
      if (memcmp(info->src_addr, pairingMac, 6) != 0) break;
    ...
    case MSG_TEXT: {
      ...
      if (memcmp(info->src_addr, chatPeerMac, 6) != 0) break;
      ...
      if(hasLastAcceptedTag && memcmp(incoming.polyTag, lastAcceptedTag, 16) == 0){
        ...
        esp_now_send(chatPeerMac, (uint8_t*)&ack, sizeof(ack));
        break;
      }
    ...
    // the else statement:
    default: {
      Serial.println("Type of packet recieved is unknown.");
        break;
    }
  }
}
...
void removeStaleDevices(){
  if(millis() - lastRemoveCheck > 1000){
    ...
    //checks for all discovered devices if last seen less than 5s ago. Discovery broadcasts every 1s, so generous.
    for(int i = 0; i < discoveredCount; i++){
      if(millis() - discoveredDevices[i].lastSeen > 5000){
    ...
    switch(deviceMode){
        case MODE_DISCOVERING:{
            ...
            if(millis() - lastBroadcastTime > 1000){
                int result = esp_now_send(broadcastAddr, (uint8_t*)&outgoing, sizeof(outgoing));
        ...
        case MODE_PENDING:{
          //after 500ms if no PairRequestReceived packet:
          if(millis() - pendingStartTime > 500){
        ...
        case MODE_CHATTING: {
          ...
          if(awaitingAck && millis() - pendingLastSendTime > ACK_TIMEOUT){
            
            //resends packet if max send attempts hasn't been exceeded
            if(pendingAttempts < MAX_SEND_ATTEMPTS){
              esp_now_send(chatPeerMac, (uint8_t*)&pendingPacket, pendingPacketLen);
              pendingAttempts++;
...
void sendEncryptedText(const char* text){
  ...
  esp_now_send(chatPeerMac, (uint8_t*)&outgoing, offsetof(TextPacket, cipherText) + textLen);
...
//ACK value is derived from session key + polyTag + "received_message" hashed. This computes it. Ensures fake ACK's cannot be spoofed.
...
void derivePairingHashes(){
  ...
  uint8_t orderedFirst[64]; //unlike sharedSecret, which returns same ans regardless of order, rand numbers wont, so we need to order them.
  uint8_t orderedSecond[64];

  if(memcmp(myRandNum, theirRandNum, 64) > 0){
    memcpy(orderedFirst, myRandNum, 64);
    memcpy(orderedSecond, theirRandNum, 64);
  } else {
    memcpy(orderedFirst, theirRandNum, 64);
    memcpy(orderedSecond, myRandNum, 64);
  }
  ...
  if(isReconnecting == false){
    ...
    hasher.update(orderedFirst, 64);
    hasher.update(orderedSecond, 64);
...
void resetPairingState(){
  ...
  memset(theirRandNum, 0 , 32);
  memset(myRandNum, 0, 64);
...
void onDiscoveryRowTap(int i){
  ...
  for(int j = 0; j < 64; j++) myRandNum[j] = esp_random() & 0xFF; //generating own randNum. 

  //adds user to peer struct 
  esp_now_peer_info_t peer = {};
  memcpy(peer.peer_addr, pairingMac, 6);
  peer.channel = 0;
  peer.encrypt = false;

  //makes sure that peer doesnt already exist before adding it as a new peer to prevent esp-now errors.
  if(!esp_now_is_peer_exist(pairingMac)) esp_now_add_peer(&peer);
...
void drawScreen(int screen) {
  switch(screen) {
    ...
    // SCREEN_KEYBOARD excluded — only entered via startKeyboard(), never via drawScreen()
  }
}
...
void setup() {
  Serial.begin(115200);
  WiFi.mode(WIFI_STA); //for esp-now and TRNG.
  ...
  //load paired and blocked devices. (necessary comment, i know).
  loadPairedDevices();
  loadBlockedDevices();

  //setting max power to transmitter and also using long range WiFi. 
  esp_wifi_set_max_tx_power(84);
  esp_wifi_config_espnow_rate(WIFI_IF_STA, WIFI_PHY_RATE_LORA_250K);

  esp_now_register_recv_cb(onDataRecv);

  //puts the device in discovery mode by making the peer MAC to the broadcast one.
  esp_now_peer_info_t peer = {};
